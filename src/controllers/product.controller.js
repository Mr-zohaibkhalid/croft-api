...
const upload = catchAsync(async (req, res) => {
  ...
  const duplicatedDescriptionIds = [];
  ...
  try {
    for (const row of products) {
      ...
      if (existingProduct) {
        ...
        const isDuplicateDescription = await checkDuplicateDescription(productData);

        if (isDuplicateDescription) {
          duplicatedDescriptionIds.push(productData.alternateID);
          failedProducts.push(productData);
        } else {
    ...
    if (duplicateAlternateIDs.length > 0 || duplicatedDescriptionIds.length > 0) {
      const errorMessage = duplicateAlternateIDs.length > 0 ?
        'Some products could not be imported due to duplicate alternate IDs' :
        'Some products could not be imported due to duplicate description rule';

      res.status(httpStatus.CREATED).send({
        status: false,
        message: errorMessage,
        importedProducts,
        duplicateAlternateIDs,
        duplicatedDescriptionIds,
        failedProducts,
      });
    } else {
      res.status(httpStatus.CREATED).send({
...
const checkDuplicateDescription = async (productData) => {
  const combinedDescription = productData.brand + productData.description + (productData.lotSize || productData.masterWeightKG);
  console.log("combinedDescription",combinedDescription)
  const existingProducts = await Product.find({});
  return existingProducts.some(existingProduct => {
    const existingCombinedDescription = existingProduct.brand + existingProduct.description + (existingProduct.lotSize || existingProduct.masterWeightKG);
    console.log("existingCombinedDescription",existingCombinedDescription)
    return existingCombinedDescription === combinedDescription;
  });
};
...
//         const existingCombinedDescription = existingProduct.brand + existingProduct.description + (existingProduct.lotSize || existingProduct.masterWeightKG);
//         if (existingCombinedDescription === combinedDescription) {
//           isDuplicate = true;
//           break;
//         }
//       }
//
//       if (isDuplicate) {
//         dublicatedDescriptionId.push(productData.alternateID);
//         failedProducts.push(productData);
//       } else {
//         const product = new Product(productData);
//         await product.save();
...
//       });
//
//       if (dublicatedDescriptionId.length > 0) {
//         res.status(httpStatus.CREATED).send({
//           status: false,
//           message: 'Some products could not be imported due to duplicate description rule',
//           importedProducts: importedProducts,
//           dublicatedDescriptionId: dublicatedDescriptionId,
//           failedProducts: failedProducts
//         });
//       }
//     } else {
